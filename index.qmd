...
where `<praat-location>` is the location of your Praat executable, `<praatsauce-location>` is the path to `praatsauce.praat`, and `<params-location>` is the path to your parameters file. If Praat is on your `PATH` and you are running the code directly from the `src` directory, it could look like this:s
...
-   `soundExt` is the file extension of sound files. Default is `wav`.
...
Kirby, James & Rasmus Puggaard-Rode (2026) *PraatSauce. Praat-based tools for spectral processing.* Version 1.0.8. Available from GitHub: [kirbyj.github.io/praatsauce](https://kirbyj.github.io/praatsauce)
